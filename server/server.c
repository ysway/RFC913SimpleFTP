...
const char checkPassword(struct Users *head, struct Users *user, const char passwd[])
{
    ...
    else
    {
        while (head)
        {
            if (!strcmp(head->passwd, passwd))
                return 2; // +Send account
            head = head->nextUser;
        }
...
const char checkFileValidity(const char path[])
{
    ...
    if (strlen(buffer) && strcmp(buffer, "0"))
        return 1;
    else
        return 0;
}
...
const char dirHandle(char workPath[], const char userArgs[])
{
    ...
    strcpy(tmpWP, workPath);
    strcpy(tmpUA, userArgs);
...
int main(int argc, char *argv[])
{
    ...
    while (1)
    {
        ...
        if ((iret = send(clientfd, messageBuffer, strlen(messageBuffer), 0)) <= 0)
        {
            perror("send");
            break;
        }
        ...
        while (1)
        {
            ...
            if (!loginVerification)
            {
                            ...
                            if (strlen(currentPass))
                            {
                                ...
                                if (!strcmp(getUserByName(&admin, userArgs)->passwd, currentPass))
                                {
                                    ...
                                    if (cdirFlag)
                                        ...
                                        strcpy(messageBuffer, "!Account valid, logged-in");
                                    loginVerification = 1;
                                }
                                else
                                {
                        ...
                        if (strlen(currentUser))
                        {
                            ...
                            if (checkPassword(&admin, getUserByName(&admin, currentUser), userArgs) == 1)
                            {
                                if (cdirFlag)
                                    ...
                                    strcpy(messageBuffer, "!Logged in");
                                loginVerification = 1;
                            }
                            else
                            {
                ...
                // Invalid command
                    ...
                    strcpy(messageBuffer, "-Invalid operation, correct spelling or try to login");
            }
            ...
            else
            {
                    ...
                    if (userArgs)
                    {
                        if (!strcmp(userArgs, "F"))
                        {
                            ...
                            if (!userArgs || dirHandle(lsPwd, userArgs))
                            {
                                ...
                                strcat(messageBuffer, lsPwd);
                                strcat(messageBuffer, ">\n");
                                while (fgets(tmpPathBuffer, sizeof(tmpPathBuffer), pathFp) != NULL)
                                {
                                    strcat(messageBuffer, "\t\t");
                                    strcat(messageBuffer, tmpPathBuffer);
                                }
                ...
                else if (!strcmp(userArgs, "CDIR"))
                {
                    ...
                    if (userArgs && strcmp(userArgs, ".") && strcmp(userArgs, "./"))
                    {
                        ...
                        strcpy(messageBuffer, "-Can’t connect to directory because: invalid input arguemnt or \".\"");
                }
                // KILL
                else if (!strcmp(userArgs, "KILL"))
                {
                    ...
                    if (userArgs)
                    {
                        ...
                        strcpy(messageBuffer, "-Invalid empty input argument");
                }
                // NAME
                else if (!strcmp(userArgs, "NAME"))
                {
                    ...
                    {
                        strcpy(messageBuffer, "-Invalid empty input argument");
                        memset(oldFileNameBuffer, 0, sizeof(oldFileNameBuffer));
                    } // reset old name
                }
                // TOBE
                else if (!strcmp(userArgs, "TOBE") && strlen(oldFileNameBuffer))
                {
                    ...
                    if (userArgs)
                    {
                        if (absolutePath)
                        {
                            strcpy(tmpPathBuffer, oldFileNameBuffer);
                            strcpy(renameBuffer, userArgs);
                        }
                        else
                        {
                            strcpy(tmpPathBuffer, workPwd);
                            strcat(tmpPathBuffer, oldFileNameBuffer);
                            strcpy(renameBuffer, workPwd);
                            strcat(renameBuffer, userArgs);
                        }
                ...
                else if (!strcmp(userArgs, "RETR"))
                {
                    ...
                    if (userArgs)
                    {
                        ...
                        if (checkFileValidity(tmpPathBuffer))
                        {
                            ...
                            memset(tmpPathBuffer, 0, sizeof(tmpPathBuffer));
                            while (fgets(tmpPathBuffer, sizeof(tmpPathBuffer), pathFp) != NULL)
                                strcpy(messageBuffer, tmpPathBuffer);
                            memset(tmpPathBuffer, 0, sizeof(tmpPathBuffer));
                            messageBuffer[strlen(messageBuffer) - 1] = 0;
                            pclose(pathFp);
                ...
                else if (!strcmp(userArgs, "SEND") && retrFound)
                {
                    ...
                    if (checkFileValidity(rsFilePathBuffer))
                    {
                        ...
                        if (strtoul(rsFileSize, 0, 10))
                        {
                            ...
                            fprintf(stdout, "[%05d]MSG: FILE CREATED\n", getpid());
                    }
                    else
                    {
                ...
                else if (!strcmp(userArgs, "STOR"))
                {
                    ...
                    if (userArgs)
                    {
                        ...
                        if (userArgs)
                        {
                            ...
                            {
                                ++p;
                                strcpy(storFilePathBuffer, workPwd);
                                strcat(storFilePathBuffer, p);
                            }
                            else
                            {
                                strcpy(storFilePathBuffer, workPwd);
                                strcat(storFilePathBuffer, userArgs);
                            }
                            p = NULL;
                            ...
                            if (!strcmp(storMode, "NEW"))
                            {
                                if (fileExist)
                                {
                                    ...
                                    strcpy(messageBuffer, "+File does not exist, will create new file");
                                /*
                            ...
                            {
                                if (fileExist)
                                    strcpy(messageBuffer, "+Will write over old file");
                                else
                                    strcpy(messageBuffer, "+Will create new file");
                            }
                            else if (!strcmp(storMode, "APP"))
                            {
                                if (fileExist)
                                    strcpy(messageBuffer, "+Will append to file");
                                else
                                    strcpy(messageBuffer, "+Will create new file");
                            }
                            else
                            ...
                            strcpy(messageBuffer, "-Invalid argument, please use { NEW | OLD | APP }");
                    }
                    else
                        strcpy(messageBuffer, "-Invalid argument, please use { NEW | OLD | APP }");
                }
                ...
                else if (!strcmp(userArgs, "SIZE") && strlen(storFilePathBuffer))
                {
                    ...
                    if (userArgs)
                    {
                        ...
                        memset(pipeArgs, 0, sizeof(pipeArgs));
                        while (fgets(tmpPathBuffer, sizeof(tmpPathBuffer), pathFp) != NULL)
                            strcpy(freeSpace, tmpPathBuffer);
                        memset(tmpPathBuffer, 0, sizeof(tmpPathBuffer));
                        ...
                        if (strtoul(freeSpace, 0, 10) > remainData)
                        {
                            ...
                            if (pathFp)
                            {
                                ...
                                strcpy(messageBuffer, "-Couldn’t save because permission denied");
                        }
                        else
                            strcpy(messageBuffer, "-Not enough room, don’t send it");
                        memset(storFilePathBuffer, 0, sizeof(storFilePathBuffer));
                        memset(storMode, 0, sizeof(storMode));
                    }
                }
                // Invalid command
                    ...
                    strcpy(messageBuffer, "-Invalid operation, please check spelling");
            }
